...
export async function createProduct(req: Request, res: Response): Promise<Response> {
    ...
    try {
        const result: Product = await createProductService(name, category_id, type_id, brand, series, technical_parameter)
        if (result) {
            channel.sendToQueue(
                "product",
                Buffer.from(
...
export async function detailProduct(req: Request, res: Response): Promise<Response> {
    const id = req.params.id
    try {
        const result: Product = await detailProductService(id)
...
export async function updateProduct(req: Request, res: Response): Promise<Response> {
    ...
    const id = req.params.id
    try {
        const result: Product = await updateProductService(id, name, category_id, type_id, brand.toUpperCase(), series.toUpperCase(), technical_parameter.toUpperCase())
        if (result) {
            channel.sendToQueue(
                "product",
                Buffer.from(
